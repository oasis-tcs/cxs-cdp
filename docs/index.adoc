...
=== Output Types and Input Types

GraphQL differentiates output types and input types as separate structures used either as part of the input query or in
the resulting output of a GraphQL query. This specification therefore specifies both input and output types. There are
also differences in terms of the features available for input and output types. For example, output types support unions
and interfaces, while input types do not. Because of these limitations the specification had to find workarounds for
some of these problems, and they will be explicitely mentioned in the sections that had to use them.

Finally, as a shortcut, GraphQL output types are just usually called GraphQL types.
...
* arraySetOfProperties([Set]) : [{"name1": "value1", "name2" : "value2"}], [{"name1": "value1", "name2" : "value2"}]
...
The arraySetOfProperties given as an example above is simply a properties of Set type but with multiple possible values.
...
Tags:: A property may be be tagged to indicate special properties such as "personalData", "requiredReadAuthorization", or
any important metadata about the property.
...
When custom EventTypes are registered in a server, new corresponding fields will be added to the in the <<CDP_EventInput>>
 type and the convention is that the field name is the same as the type name but starting with a lowercase character
...
input CDP_EventInput {
  ...
  vENDORPageViewEvent : VENDOR_PageViewEventInput # note the lowercase v as a convention as well as the absence of the "Input" suffix.
}
----
