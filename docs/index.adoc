...
Below is a high level introduction to the domain objects of the specification:

=== Events

Events represent user behavioral data/interactions.
As such, events are always associated with <<Profiles>>.
A transaction, a page view, a click on a link all represents different events.
The targets of an interaction is known as <<Objects>>
Events are fed to the Context Server from different sources.
CDP implementations may define custom event types, in addition to the mandatory event types.

=== Profiles

Profiles represent "individuals/visitors/customers".
Administrators may define available properties for profiles.
As such, profiles may compose properties from a range of different sources.
Profiles are created and updated through events.

=== Objects

Objects are representations of any item/person that <<Profiles>> interact with through <<Events>>.
The interaction with objects enables the CDP provide to detect interests, perform optimizations and recommendations if implemented.

=== Lists

Lists enable manual or programmatic grouping of profiles.
A list may for instance represent participation in a newsletter, a selection of profiles for a marketing campaign, or blacklisting.
Lists also support opt-out, effectively preventing profiles from being re-added to a list.

=== Segments

Segments are similar to lists, but a profile's inclusion is here defined through a rule.
More specifically, administrative users may define so-called filters that produce a set of matching <<Profiles>> based on previous events or specific properties.

=== Concents

<<Profiles>> may grant or revoke concents for any specific purpose defined by the CDP administrators.
This helps organizations document and stay in compliance with legal requirements for personal data,
as well as managing concents consistently across all systems and touch points.

=== Other

Other important domain objects are:

Properties: Lets administrators configure custom profile properties based on a range of value types
Clients:: A client is required in order to communicate with the CDP. Clients are responsible for uniquely identifying its profiles.
Views:: Administrative users may compose views where objects such as lists and segments are stored, easing use for distributed organisations.
Topics:: Topics represent top level areas of interest of the organisation. This might for instance be a product, a location or a product group.
Filters:: A solution to consistently perform queries on the various CDP objects
...
Here's an example of a `query operation:
...
[options="header"]
|=========================================================================
|Operators |Identifier|String|Int|Float|Date|Boolean|GeoPoint|Enumeration|
...
Sources provide a way to identify the exact origin of events beyond the client.
...
[source,graphql]
----
include::{sourcedir}/schemas/events.js[lines=46..46]
----
...
A Customer Data Platform dynamically builds profiles from events that occur over time, transparently creating profiles
...
[source,graphql]
----
include::{sourcedir}/schemas/profiles.js[lines=40..50]
----
...
TODO: Make this version inline rather than what we see below?
...
[source,graphql]
----
include::{sourcedir}/schemas/profiles.js[lines=60..61]
----
...
[source,graphql]
----
include::{sourcedir}/schemas/profiles.js[lines=68..68]
----
...
[source,graphql]
----
include::{sourcedir}/schemas/profiles.js[lines=72..72]
----

=== Profile properties

Each deployment of CDP will be unique in how data are collected, and what data is stored per profile.
Profile properties enable us to define all custom properties required by an organization.
...
The specification does not define a set of standard profile properties. However, implementors are recommended to consider including standard properties such as :

- full name : string
- primary email : identifier
- primary phone number : identifier
- date of birth : date
- gender : string
- etc.
...
[source,graphql]
----
include::{sourcedir}/schemas/profiles.js[lines=62..62]
----
...
[source,graphql]
----
include::{sourcedir}/schemas/profiles.js[lines=66..67]
----
...
[source,graphql]
----
include::{sourcedir}/schemas/profilefilters.js[lines=9..14]
----
...
[source,graphql]
----
include::{sourcedir}/schemas/profilefilters.js[lines=19..23]
----
...
[source,graphql]
----
include::{sourcedir}/schemas/profilefilters.js[lines=31..38]
----
...
[source,graphql]
----
include::{sourcedir}/schemas/profilefilters.js[lines=40..47]
----
...
The CDP_SessionStateEventInput is used to signify the beginning, pause, resume or end of a session.
...
==== CDP_SessionStateEvent

[source,graphql]
----
include::{sourcedir}/schemas/sessions.js[lines=9..19]
----

==== CDP_SessionStateEventInput

[source,graphql]
----
include::{sourcedir}/schemas/sessions.js[lines=21..23]
----
...
[source,graphql]
----
include::{sourcedir}/schemas/consents.js[lines=22..27]
----
...
[source,graphql]
----
include::{sourcedir}/schemas/consents.js[lines=30..32]
----
