...
If custom EventTypes are registered in a server, when using as fields in the EventInput type, the convention is that
the field name is the same as the type name but starting with a lowercase character instead of an uppercase one.
...
Once the event types are defined (in an implementation-specific manner), they can be send using the `processEvents` mutation field
that uses the <<CDP_EventInput>> type and queried using the `findEvents` query field
...
TODO Should profile properties mutations be made optional ? Implementations could provide their own way of registering properties and not want to expose this ?
