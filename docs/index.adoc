...
Filters are used in the Context Server to drill down into profiles and events.
Filters were designed to be flexible to implement, but also easy to use for administrators and marketeers in a visual user interfaces.

Filters are essentially queries that operate on properties.
Basic expressions can be chained with the boolean operators "AND" or "OR".
TODO: List all operators

A propertyFilter might be is composed of the propertyName concatenated with a property operator. The available operators
depend on the property value type. For example for a string value type the following operators will be applied:
...
So in this example, if we have a property named "firstName" of value type string, the following property filters will
be generated in the GraphQL schema:
...
In this case, the value of the GraphQL field will be a simple string value, but in some more complex value types such
as a geopoint, the value might be more complex, as in the following example:
...
Use EventFilters to search for specific events registered in the CXS.
EventFilters let your query based on the following:
...
Profile Filters are more complex than <<EventFilters>>.
A profileFilter enables you to query based on the following:

* Profile properties
* Segments that the profile matches
* Consents granted by the profile
* Events - though an eventFilter
