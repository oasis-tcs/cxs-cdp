...
TODO Quickly describe main domain objects (1-2 lines per object)
...
GraphQL requests are usually composed of two parts : query and variables.

Throughout this document we will provide GraphQL request examples in the following form:

    operation:
        GraphQL query, mutation or subscript
    variables:
        JSON structure
...
GraphQL request. Here's an example of the varible part:
...
Objects are used in <<Events>> to specify what the <<Profiles>> is interacting with and also used in <<Optimizations>>. Objects may optionally be part of one or multiple collections, that are used to classify them. By placing objects into collections, optimizations may be done within a single collection (e.g. : recommending products).
...
[source,graphql]
----
include::{sourcedir}/schemas/consents.js[lines=8..20]
----
