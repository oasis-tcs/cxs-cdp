...
to first retrieve the HTML needed to deliver the page content, and then the CDP to get and update information about the
profile of the current visitor to the site. The CMS may contain personalization filters that are sent to the CDP for
evaluation and will allow the CMS to personalize the page content based on whether the current visitor profile matches
the filters or not.
...
In the above illustration, this use case is implemented by using a CMS to deliver the different variants of content, and an
algorithm is used to perform the random display distribution. Once the variant is selected and displayed, the CDP receives
an event that updates the visitor profile with the variant that has been selected and displayed. Later on, this information may be
used to classify the visitor profile into a subset (of profiles) or to personalize a future interaction with the systems
...
[source,graphql]
----
include::{sourcedir}/schemas/views.js[lines=2..9]
----
...
[source,graphql]
----
include::{sourcedir}/schemas/lists.js[lines=2..15]
----
